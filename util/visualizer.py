...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=140, savedir=None, show_in_console=True):
        ...
        ax = plt.gca()
        img = renorm(img).permute(1, 2, 0).cpu()
    ...
    def addtgt(self, tgt):
        ...
        boxes = []
        color_dict = dict()
        for unnormbbox, label in zip(tgt['boxes'].cpu(), tgt['box_label']):
            ...
            if label.split('_')[0] not in color_dict:
                c = (np.random.random((1, 3))*0.6+0.4).tolist()[0]
                color_dict[label.split('_')[0]] = c
            color.append(color_dict[label.split('_')[0]])
